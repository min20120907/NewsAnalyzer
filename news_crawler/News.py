...
class News:
    ...
    def kw(self, title, content_str):
        ...
        list_sentiment = []
        interval = ','
        tags1 = jieba.analyse.extract_tags(
            title, topK=3, withWeight=True, allowPOS=False)

        for tag in tags1:
            list_title_kw.append(tag[0])  # 抓出標題關鍵字
        str1 = interval.join(list_title_kw)  # 標題關鍵字逗號隔開
        tags2 = jieba.analyse.extract_tags(
            content_str, topK=3, withWeight=True, allowPOS=False)  # topK=x,抓出3個最相關
