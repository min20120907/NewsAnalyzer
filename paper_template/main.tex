...
%======================================================================
\begin{abstract}
This paper presents NewsAnalyzer, a real-time fake news detection system that deploys a local large language model directly on client hardware. NewsAnalyzer conducts multi-dimensional credibility analysis---including structured viewpoint synthesis, inquiry targets, and contextual credibility scoring---without relying on remote cloud APIs, thereby eliminating network latency overheads and user privacy risks. Unlike standard cloud API deployments, our system enforces strict JSON output via Ollama's grammar-constrained decoding (\texttt{format:\allowbreak json}), eliminating reasoning artifacts (e.g., \texttt{<thought>} tags) that trigger \texttt{JSONDecodeError} exceptions. The system integrates Sentence-BERT (SBERT) semantic embeddings over a localized Traditional Chinese fact-checking corpus (Cofacts) for nearest-neighbor retrieval. The resulting multi-signal assessment is rendered directly in the social feed via a Chrome Extension, providing robust misinformation deterrence for the Traditional Chinese social media ecosystem.
...
\begin{tikzpicture}[
  ...
  \node[box, below right=0.45cm and 0.4cm of prompt] (constrained) {Ollama Engine\\(\texttt{format:json}, \texttt{think:false})};

  \node[box, right=0.4cm of unconstrained] (fail) {\texttt{JSONDecodeError}\\(15--20\% failure rate)};
